...
rainbow = [0x0E0001, 0x0E0600, 0x0F0900, 0x0F0C00, 0x0F0F00, 0x0C0D00, 0x080C01, 0x020A03,
           ...
           0x010208, 0x060108, 0x090008, 0x0B0008, 0x0E0007, 0x0E0006, 0x0E0004, 0x0E0003] * 10
# rainbow = (rainbow + rainbow)[::2]
LEN_RIBBON = 110
HALF_RIBBON = LEN_RIBBON // 2
DEC_RIBBON = LEN_RIBBON - 1

def stream():
    for i in range(23, -1, -1):
        yield rainbow[i:i+LEN_RIBBON]

def wave():
    for i in range(DEC_RIBBON):
        yield rainbow[:i] + [0] * (LEN_RIBBON - i)
    for i in range(DEC_RIBBON):
        yield [0] * i + rainbow[i:]

def ping_pong():
    for i in range(12):
        yield rainbow[12 - i:24] + [0] * (LEN_RIBBON - 12 + i)
    for i in range(LEN_RIBBON-12):
        yield ([0] * i + rainbow[:24] + [0] * LEN_RIBBON)[:LEN_RIBBON]
    for i in range(LEN_RIBBON-12):
        yield ([0] * (LEN_RIBBON - 12  - i) + rainbow[:24] + [0] * LEN_RIBBON)[:LEN_RIBBON]
    for i in range(12):
        yield rainbow[i:24] + [0] * (LEN_RIBBON - 24 + i)

def in_out():
    yield [0] * LEN_RIBBON
    for i in list(range(1, LEN_RIBBON)) + list(range(LEN_RIBBON, 0, -1)):
        yield rainbow[-i:] + [0] * (LEN_RIBBON - i)

def spring():
    for i in range(23, -1, -1):
        yield rainbow[i:i+HALF_RIBBON][::-1] + rainbow[i:i+HALF_RIBBON]
